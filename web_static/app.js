const state = {events: [], completed: {}, configured: false, source: '', last_sync: null, csrf: '', name: ''};
let view = 'upcoming';
let busy = false;
...
function render() {
  const now = new Date();
  $('#account-name').textContent = state.name ? `${state.name}님` : '';
...
$('#logout-button').addEventListener('click', async () => {
  try { await api('/logout', {}); window.location.href = '/welcome'; }
  catch (error) { showMessage(error.message); }
});
(async () => {
  try {
    ...
    if (error.message === '로그인이 필요합니다.') window.location.href = '/welcome';
